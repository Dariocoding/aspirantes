/** Rate limit en memoria para el portal público de actualización por cédula. */
...
export function isSelfServiceIpLocked(ip: string): boolean {
...
export function recordSelfServiceFailure(ip: string): void {
  const now = Date.now();
...
export function clearSelfServiceFailures(ip: string): void {
  attempts.delete(ip);
}
