...
import { createAspirante, updateAspirante } from "@/app/actions/aspirantes";
import type { AspiranteActionState } from "@/lib/action-types";
import { aspiranteInitialActionState } from "@/lib/action-types";
...
import { AspiranteFichaEvaluacionFields } from "@/components/forms/aspirante-ficha-evaluacion-fields";
...
function ErrorList({ errors }: { errors: Record<string, string> }) {
  ...
  return (
    <>
      {formMsg ? (
        <p className="md:col-span-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{formMsg}</p>
      ) : null}
      {entries.length ? (
...
const STEPS = [
  { title: "Identidad", description: "Nombre, unidad postulante, admisión y nacimiento" },
  ...
  { title: "Expediente y evaluaciones", description: "Revisión, prueba física y examen médico" },
] as const;
...
export function AspiranteRegistroForm({
  ...
  const serverAction = useMemo(() => (isEdit ? updateAspirante : createAspirante), [isEdit]);
  const [state, formAction, isPending] = useActionState<AspiranteActionState, FormData>(
    serverAction,
    aspiranteInitialActionState,
  );
  ...
  function validateStep(index: number) {
    ...
    const fs = form.querySelector(`fieldset[data-registro-step="${index}"]`) as HTMLFieldSetElement | null;
    if (!fs) return false;
    fs.hidden = false;
  ...
  return (
    <>
      ...
      onSubmit={(e) => {
        ...
        for (let i = 0; i < STEPS.length; i++) {
          const fs = form.querySelector(`fieldset[data-registro-step="${i}"]`) as HTMLFieldSetElement | null;
          if (!fs) continue;
          fs.hidden = false;
          if (!fs.checkValidity()) {
            e.preventDefault();
    ...
    >
      {initial?.id ? <input type="hidden" name="aspiranteId" value={initial.id} /> : null}

      <nav aria-label="Pasos del formulario" className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
        {STEPS.map((s, i) => {
          const stepperItem = (
            <>
              <span
                className={cn(
                  ...
                  step === i ? "bg-white/20 text-white" : "bg-slate-100 text-slate-700",
                )}
              >
                {i + 1}
              </span>
              <span className="min-w-0">
                ...
                <span className={cn("block truncate", step === i ? "text-slate-200" : "text-slate-500")}>
                  {s.description}
                </span>
              </span>
            </>
          ...
          const stepperClass = cn(
            ...
            isEdit && step !== i ? "cursor-pointer hover:border-slate-300 hover:bg-slate-50" : "cursor-default",
          );

          if (isEdit) {
            return (
              <button key={s.title} type="button" onClick={() => setStep(i)} className={stepperClass}>
                {stepperItem}
              </button>
            );
          }
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {convocatoriaActiva ? (
          <p className="md:col-span-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
            <span className="font-medium text-slate-900">Convocatoria activa:</span> {convocatoriaActiva.nombre}{" "}
            <span className="font-mono text-xs text-slate-500">({convocatoriaActiva.codigo})</span>
          </p>
        ) : (
          <p className="md:col-span-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
            No hay convocatoria activa. Un administrador debe definir y activar un período de ingreso en{" "}
            <span className="font-medium">Convocatorias</span> antes de registrar aspirantes.
          </p>
        )}
      </div>

      <fieldset data-registro-step={0} hidden={step !== 0} className="min-w-0 space-y-3 border-0 p-0" aria-labelledby="step-1-title">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <Label>Nombres</Label>
            <Input name="nombres" required defaultValue={defaults.nombres} autoComplete="given-name" />
          </div>
          <div>
            <Label>Apellidos</Label>
            <Input name="apellidos" required defaultValue={defaults.apellidos} autoComplete="family-name" />
          </div>
          <div>
            <Label>Cédula</Label>
            <Input name="cedula" required inputMode="numeric" defaultValue={defaults.cedula} />
          </div>
          <div>
            <Label>Sexo</Label>
            <select
          ...
          <div>
            ...
            <Input name="fechaNacimiento" type="date" required defaultValue={defaults.fechaNacimiento} />
          </div>
          <div>
      ...
      <fieldset data-registro-step={1} hidden={step !== 1} className="min-w-0 space-y-3 border-0 p-0" aria-labelledby="step-2-title">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <Label>Edad</Label>
            <Input name="edad" type="number" required defaultValue={defaults.edad} />
          </div>
          <div>
            <Label>Hijos</Label>
            <Input name="hijosCantidad" type="number" defaultValue={defaults.hijosCantidad} />
          </div>
          <div>
      ...
      <fieldset data-registro-step={2} hidden={step !== 2} className="min-w-0 space-y-3 border-0 p-0" aria-labelledby="step-3-title">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            ...
            <Input name="estaturaCm" type="number" step="0.01" defaultValue={defaults.estaturaCm} />
          </div>
          <div>
            <Label>Peso (kg)</Label>
            <Input name="pesoKg" type="number" step="0.01" defaultValue={defaults.pesoKg} />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <Textarea name="condicionesMedicas" defaultValue={defaults.condicionesMedicas} />
          </div>
          <div>
      ...
      <fieldset data-registro-step={3} hidden={step !== 3} className="min-w-0 space-y-3 border-0 p-0" aria-labelledby="step-4-title">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div>
            ...
            <Input name="contactoParentesco" required defaultValue={defaults.contactoParentesco} />
          </div>
          <div>
            <Label>Teléfono de Emergencia</Label>
            <Input name="contactoTelefono" type="tel" required defaultValue={defaults.contactoTelefono} />
          </div>
          <div className="md:col-span-2">
            <Label>Dirección de Emergencia</Label>
            <Textarea name="contactoDireccion" defaultValue={defaults.contactoDireccion} />
          </div>
        </div>
      </fieldset>

      <fieldset data-registro-step={4} hidden={step !== 4} className="min-w-0 space-y-3 border-0 p-0" aria-labelledby="step-5-title">
        <p id="step-5-title" className="sr-only">
          Paso 5: {STEPS[4].title}
        </p>
        <p className="text-xs text-slate-600">
          Los requisitos y tablas provienen de un catálogo en sistema: al actualizar listados, los formularios nuevos reflejan
          los cambios sin migraciones de columnas.
        </p>
        <AspiranteFichaEvaluacionFields initialJson={initial?.fichaEvaluacion ?? null} />
      </fieldset>
      ...
      <div className="flex flex-col-reverse gap-2 border-t border-slate-200/90 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-slate-500">
          Paso <span className="font-semibold tabular-nums text-slate-800">{step + 1}</span> de {STEPS.length}
        </p>
