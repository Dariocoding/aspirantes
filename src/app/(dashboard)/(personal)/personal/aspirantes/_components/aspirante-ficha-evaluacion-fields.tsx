    ...
    <div className="flex flex-wrap justify-center gap-3 sm:justify-end" role="group" aria-label={`Requisito ${id}`}>
      {(
        [
          ...
          { key: "SI", label: "SI", val: "SI" as const },
    ...
    <div className="space-y-8">
      ...
      <section className="min-w-0 space-y-3" aria-labelledby="ficha-a-title">
        <div>
          ...
          <p className="text-xs text-slate-600">Marque SI o NO por requisito. Use «—» si aún no aplica.</p>
        </div>
          ...
          <table className="w-full min-w-0 table-fixed border-collapse text-sm">
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="px-2 py-2 text-left text-xs font-semibold uppercase text-slate-600">NÂ°</th>
            ...
            return (
              <div key={def.id} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                ...
                <p className="mt-3 border-t border-slate-100 pt-2 text-[11px] text-slate-500">
                  ...
                  {" Â· "}
                  Ref. subt. <span className="font-medium text-slate-700">{def.refSubtotal || "â€”"}</span>
                </p>
              </div>
            );
          })}
              ...
              {PRUEBA_FISICA_FILAS.map((def) => {
                ...
                return (
                  <tr key={def.id} className="border-b border-slate-100 align-top last:border-0">
                    ...
                    <td className="px-2 py-2 text-center text-xs text-slate-500">{def.refSubtotal || "â€”"}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <section className="min-w-0 space-y-3" aria-labelledby="ficha-c-title">
        <div>
          ...
          <p className="text-xs text-slate-600">SI / NO y observaciones por ítem.</p>
        </div>
        <div className="space-y-3 md:hidden">
        ...
        <div className="hidden overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm md:block">
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="w-10 px-2 py-2 text-left text-xs font-semibold uppercase text-slate-600">NÂ°</th>
                <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-slate-600">Prueba / estudio</th>
                <th className="w-28 px-2 py-2 text-center text-xs font-semibold uppercase text-slate-600">SI</th>
