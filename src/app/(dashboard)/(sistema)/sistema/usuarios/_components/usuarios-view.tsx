...
} from "@dashboard/usuarios/_components/usuario-modals";
...
function roleBadgeClass(roleKey: string, isSuper: boolean) {
  if (isSuper) return "border-amber-300 bg-amber-50 text-amber-950";
  switch (roleKey) {
    case "ADMIN":
...
type Props = {
  ...
  currentUserRoleIsSuper: boolean;
  canManageUsers: boolean;
  assignableRoles: AuthRoleSummary[];
};

function canManageRow(actorIsSuper: boolean, canManageUsers: boolean, targetIsSuper: boolean) {
      ...
      <Card className="gap-0 py-0 shadow-sm shadow-slate-900/5 ring-slate-200/80">
        <CardHeader className="border-b border-slate-200/80 bg-linear-to-br from-slate-50 to-white px-4 py-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 flex-1 gap-2.5">
              ...
              <div className="min-w-0 space-y-0.5">
                ...
                <CardDescription className="text-xs leading-snug text-slate-600">
                  Cuentas y roles configurables. No se puede crear un super administrador desde la interfaz.
            ...
            {canManageUsers ? <UsuarioCreateDialog assignableRoles={assignableRoles} /> : null}
          </div>
        </CardHeader>
      </Card>
              ...
              <TableBody>
                {users.length === 0 ? (
                  ...
                  users.map((u) => {
                    ...
                    const canManage = canManageRow(currentUserRoleIsSuper, canManageUsers, u.roleIsSuper);
                    return (
                        ...
                        <TableCell className="px-4 py-3">
                          <span
                            className={cn(
                              ...
                              roleBadgeClass(u.roleKey, u.roleIsSuper),
                            )}
                          >
